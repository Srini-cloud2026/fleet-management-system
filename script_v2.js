...
async function loadVehicleData() {
    ...
    try {
        ...
        if (topBar) {
            if (!document.getElementById('debug-version')) {
                topBar.insertAdjacentHTML('beforeend', `<span id="debug-version" style="font-size:10px; margin-left:10px; opacity:0.5; color: var(--text-muted)">v2.8-Live</span>`);
            } else {
                document.getElementById('debug-version').textContent = 'v2.8-Live';
            }
        }
...
function renderFleetGrid(categoryFilter = null) {
    ...
    const categories = [...new Set(vehicleMasterData
        .map(v => {
            const val = v['Category '] || v['category'] || v['CATEGORY'] || 'Other';
            return val.toString().trim().toLowerCase();
        })
    )];
    
    if (categories.length === 0 && vehicleMasterData.length > 0) {
        console.error("No categories found in vehicle data!");
    }
    ...
    categories.forEach(cat => {
        ...
        const displayName = firstMatch ? (firstMatch['Category '] || firstMatch['category'] || firstMatch['CATEGORY'] || 'Other').toString().trim() : 'Other';
