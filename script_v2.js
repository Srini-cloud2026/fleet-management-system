...
async function loadVehicleData() {
    alert("ANTIGRAVITY_V2_LOADED");
    console.log("Loading vehicle data...");
    
    // Check if data is already loaded via <script> tag (fallback for file:// CORS issues)
    if (typeof VEHICLE_MASTER_DATA !== 'undefined') {
        vehicleMasterData = VEHICLE_MASTER_DATA;
        console.log("Found global VEHICLE_MASTER_DATA:", vehicleMasterData.length);
        if (typeof DRIVER_MASTER_DATA !== 'undefined') {
            driverMasterData = DRIVER_MASTER_DATA;
            console.log("Found global DRIVER_MASTER_DATA:", driverMasterData.length);
        } else {
            console.warn("DRIVER_MASTER_DATA not found in global scope!");
        }
        console.log("Total vehicles:", vehicleMasterData.length, "Total staff:", driverMasterData.length);
    ...
    try {
        const response = await fetch('vehicle_master.json');
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const allData = await response.json();
        
        // Handle multi-sheet format or single-sheet
        if (allData.Sheet1) {
            vehicleMasterData = allData.Sheet1;
            driverMasterData = allData['Drivers Info'] || [];
        } else {
            vehicleMasterData = Array.isArray(allData) ? allData : (allData.Sheet1 || []);
            driverMasterData = allData['Drivers Info'] || [];
        }
        
        console.log("Data loaded successfully. Vehicles:", vehicleMasterData.length, "Staff:", driverMasterData.length);
...
function handleLogin(e, role) {
    ...
    if (role === 'driver') {
        ...
        if (passcode === '1234' && (VALID_DRIVERS[truckId] || vehicleMasterData.some(v => v['PLATE NO'].toString().includes(truckId)))) {
            currentUser = {
                id: truckId,
                empId: empId,
                name: driverName || (VALID_DRIVERS[truckId] ? VALID_DRIVERS[truckId].name : 'Driver ' + truckId)
            };
            currentRole = 'driver';
            success = true;
            initDriverHub();
...
async function fetchGpsData() {
    ...
    try {
        ...
        gpsData = newGpsData;
        console.log("GPS data synced for", Object.keys(gpsData).length, "vehicles.");
        
        // Refresh current views if they show GPS info
        if (currentRole === 'admin') {
            if (currentCategory) renderCategoryVehicles(currentCategory);
            // Update vehicle modal if open
            const modal = document.getElementById('vehicle-modal');
            if (modal && modal.classList.contains('active')) {
                const plateText = document.getElementById('modal-vehicle-title').textContent;
                const plate = plateText.replace('Vehicle: ', '');
                updateVehicleModalGpsInfo(plate);
            }
        }
    } catch (error) {
        ...
        gpsContainer.innerHTML = `
            <div class="detail-card glass-effect gps-highlight">
                ...
                <ul class="detail-list">
                    ...
                    <li><span class="label">Odometer</span> <span class="value" style="font-weight:700; color:var(--accent-blue)">${live.odometer.toLocaleString()} km</span></li>
                </ul>
            </div>
        `;
    } else {
...
function updateVehicleKPIs() {
    ...
    const totalVehiclesEl = document.querySelector('.kpi-total-vehicles');
    if (totalVehiclesEl) totalVehiclesEl.textContent = totalVehicles;

    // Active Vehicles (User says all 190 are active)
    const activeVehicles = totalVehicles;
    const activeVehiclesEl = document.querySelector('.kpi-active-vehicles');
    ...
    const driversCount = driverMasterData.filter(d => !d["Employee's Name"].toLowerCase().includes('mechanic')).length;
    ...
    vehicleMasterData.forEach(v => {
        ...
        if (v['VALID DAYS'] < 30) {
            expiryItems.push({
                ...
                type: 'Mulkiya',
                date: v['MULKIYA EXP DATE'],
                days: v['VALID DAYS']
            });
        }
        // Insurance
        if (v['VALID DAYS.1'] < 30) {
            expiryItems.push({
                plate: v['PLATE NO'],
                category: v['Category '] || 'N/A',
                type: 'Insurance',
                date: v['INSURANCE EXP DATE'],
                days: v['VALID DAYS.1']
            });
        }
    });
...
function renderFleetGrid(categoryFilter = null) {
    ...
    // Generate unique categories (case-insensitive)
    const categories = [...new Set(vehicleMasterData.map(v => v['Category '].toString().trim().toLowerCase()))];
    
    categories.forEach(cat => {
        const matchingVehicles = vehicleMasterData.filter(v => v['Category '].toString().trim().toLowerCase() === cat);
        const displayName = matchingVehicles[0]['Category '].toString().trim(); // Use original casing from first match
...
function renderCategoryVehicles(categoryLower) {
    ...
    const vehicles = vehicleMasterData.filter(v => v['Category '].toString().trim().toLowerCase() === categoryLower);
    ...
    vehicles.forEach(v => {
        const plate = v['PLATE NO'];
        const account = v['Account '] || 'N/A';
        const insuranceExpiry = v['INSURANCE EXP DATE'];
        const mulkiyaExpiry = v['MULKIYA EXP DATE'];
        const insuranceDays = v['VALID DAYS.1'];
        const mulkiyaDays = v['VALID DAYS'];
        ...
        // GPS status check
        const live = gpsData[plate];
        const gpsIndicator = live ? 
            `<span class="gps-badge ${live.ignition ? 'moving' : 'parked'}"><i class="fas fa-satellite"></i> Live</span>` : '';

        const card = `
            <div class="vehicle-card" onclick="showVehicleDetail('${plate}')">
                <div class="vehicle-card-header">
                    <div class="plate-number">${plate} ${gpsIndicator}</div>
                ...
                <div class="vehicle-card-body">
                    ...
                    ${live ? `
                    <div class="info-row gps-mini">
                        <i class="fas fa-tachometer-alt"></i>
                        <div class="info-content">
                            <span class="info-label">Odometer</span>
                            <span class="info-value highlight">${live.odometer.toLocaleString()} km</span>
                        </div>
                    </div>` : ''}
                </div>
                <div class="vehicle-card-footer">
                    <span class="vehicle-type-tag">${v['VEHICLE TYPE']}</span>
                </div>
            </div>
        `;
...
function showVehicleDetail(plateNo) {
    const vehicle = vehicleMasterData.find(v => v['PLATE NO'].toString() === plateNo);
    if (!vehicle) return;
    ...
    title.textContent = `Vehicle: ${vehicle['PLATE NO']}`;
    ...
    switch (type) {
        ...
        case 'manpower-roles':
            ...
            driverMasterData.forEach(d => {
                if (d["Employee's Name"].toLowerCase().includes('mechanic')) mCounts['Mechanics']++;
        ...
        case 'expiring-policies':
            ...
            vehicleMasterData.forEach(v => {
                if (v['VALID DAYS'] >= 0 && v['VALID DAYS'] < 30) pCounts['Mulkiya']++;
                if (v['VALID DAYS.1'] >= 0 && v['VALID DAYS.1'] < 30) pCounts['Insurance']++;
            });
...
function renderManpowerTable() {
    ...
    if (currentManpowerFilter === 'driver') {
        filteredData = driverMasterData.filter(d => !d["Employee's Name"].toLowerCase().includes('mechanic'));
    } else if (currentManpowerFilter === 'mechanic') {
        filteredData = driverMasterData.filter(d => d["Employee's Name"].toLowerCase().includes('mechanic'));
    } else if (currentManpowerFilter === 'expiring') {
        // Since we don't have real dates, for demo we'll show none or mock some
        filteredData = []; 
    }

    filteredData.forEach(d => {
        const name = d["Employee's Name"];
        const isMechanic = name.toLowerCase().includes('mechanic');
...
function renderAttendanceTable() {
    ...
    driverMasterData.slice(0, 15).forEach(d => {
        const row = `
            <tr>
                <td style="font-weight:600">${d["Employee's Name"]}</td>
                <td>06:00 AM</td>
                <td>—</td>
                <td>4.5h</td>
                <td>—</td>
