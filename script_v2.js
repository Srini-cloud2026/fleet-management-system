...
const SUPABASE_KEY = 'sb_publishable_AmjcxwDgNpJLnJV6A7YlYw_Y-7n4olu';
...
async function loadVehicleData() {
    ...
    try {
        ...
        // Fetch Drivers from Driver_master
        const { data: drivers, error: dError } = await supabaseClient
            .from('Driver_master')
            .select('*');
...
async function fetchGpsData() {
    ...
    try {
        ...
        if (topBar && !document.getElementById('debug-version')) {
            topBar.insertAdjacentHTML('beforeend', `<span id="debug-version" style="font-size:10px; margin-left:10px; opacity:0.5; color: var(--text-muted)">v2.6-Live</span>`);
        }
        
        console.log("GPS data synced via Proxy (Backend logging enabled).");
